...
NAV2D.Navigator = function(options) {
  ...
  function sendGoal(pose) {
    ...
    navgoal.on('result', function() {
      that.rootObject.removeChild(that.goalMarker);
    });
  }

  function sendInit(pose) {
    // create a goal
  ...
  if (withOrientation === false){
    ...
    // setup a aux click listener (no orientation)
...
NAV2D.OccupancyGridClientNav = function(options) {
  ...
  var navtopic = options.navtopic || '/move_base_simple/goal';
  var navtopic_msgtype = options.navtopic_msgtype || 'geometry_msgs/PoseStamped';
  var rootObject = options.rootObject || new createjs.Container();
  var viewer = options.viewer;
  var withOrientation = options.withOrientation || false;
  var image = options.image || false;
